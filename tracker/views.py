...
def _user_categories(user, category_type=None):
    qs = Category.objects.filter(
        owner__isnull=True) | Category.objects.filter(owner=user)
    qs = qs.distinct()
    if category_type:
...
def dashboard(request):
    ...
    try:
        ...
        total_balance = sum(
            (pm.current_balance for pm in payment_methods), Decimal("0"))
    except Exception as e:
    ...
    try:
        ...
        total_income = month_txns.filter(txn_type=Transaction.INCOME).aggregate(
            s=Sum("amount"))["s"] or Decimal("0")
        total_expense = month_txns.filter(txn_type=Transaction.EXPENSE).aggregate(
            s=Sum("amount"))["s"] or Decimal("0")
        ...
        recent_transactions = month_txns.select_related(
            "category", "payment_method")[:20]
        recent_transfers = Transfer.objects.filter(
            owner=request.user).select_related("from_method", "to_method")[:10]
    except Exception as e:
...
def yearly_overview(request):
    ...
    for m in range(1, 13):
        txns = Transaction.objects.filter(
            owner=request.user, date__year=year, date__month=m)
        income = txns.filter(txn_type=Transaction.INCOME).aggregate(
            s=Sum("amount"))["s"] or Decimal("0")
        expense = txns.filter(txn_type=Transaction.EXPENSE).aggregate(
            s=Sum("amount"))["s"] or Decimal("0")
        monthly_data.append({
    ...
    total_income = year_txns.filter(txn_type=Transaction.INCOME).aggregate(
        s=Sum("amount"))["s"] or Decimal("0")
    total_expense = year_txns.filter(txn_type=Transaction.EXPENSE).aggregate(
        s=Sum("amount"))["s"] or Decimal("0")
...
def transfer_money(request):
    ...
    if request.method == "POST":
        from_method = get_object_or_404(
            methods, pk=request.POST.get("from_method"))
        to_method = get_object_or_404(
            methods, pk=request.POST.get("to_method"))
        try:
            amount = Decimal(request.POST.get("amount", "0"))
            ...
            messages.error(
                request, "Check the accounts and enter an amount within the available balance.")
        else:
...
def transactions(request):
    qs = Transaction.objects.filter(owner=request.user).select_related(
    ...
    if q:
        qs = qs.filter(note__icontains=q) | qs.filter(
...
def export_csv(request):
    ...
    for t in Transaction.objects.filter(owner=request.user).select_related("category", "payment_method"):
        writer.writerow([t.date, t.get_txn_type_display(
        ), t.category.name, t.payment_method.name, t.amount, t.note])
    return response
...
"""
            ...
            logger.info(
                f"Verification code sent to {email}: {verification_code}")
            ...
            messages.error(
                request, f'Could not send verification email. Error: {str(e)}')
            return render(request, 'tracker/signup.html')
    ...
    if request.method == 'POST':
        ...
        if code == expected_code:
            try:
                ...
                messages.success(
                    request, 'Account created successfully! Welcome to Amar Hishab.')
                return redirect('tracker:dashboard')
            ...
            request.session[attempt_key] = request.session.get(
                attempt_key, 0) + 1
            if request.session[attempt_key] >= 3:
                messages.error(
                    request, 'Too many failed attempts. Please sign up again.')
                if 'signup_data' in request.session:
                    del request.session['signup_data']
                if 'signup_email' in request.session:
                    del request.session['signup_email']
            ...
            messages.error(
                request, f'Invalid verification code. Attempt {request.session[attempt_key]} of 3.')
            return render(request, 'tracker/verify_email.html', {'email': email})
